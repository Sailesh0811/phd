...
$.ajax({
	type:'post',
	url:'php/candmessages.php',
	success: function(data){
		details = JSON.parse(data);
                console.log(details);
                if(details=="error"){
                	$('#comments').append('<div class=row><div class="col  s12"><div class="card-panel blue-grey"><span class=white-text><p><strong><b>No Requests</b></strong><p id=school></span></div></div></div>');
                }else{
        ...
        var str='<div class="col  s4"><div class="card-panel blue-grey darken-1"><span class=white-text><p><strong>';
        str+='</b></strong></p><p>';
        str+=canddetails+'</p></span></div></div>';
        $('#comments').append(str);
	}
}
}
...
$.ajax({
  type:'post',
  url:'php/candmeetings.php',
  success: function(data){
    details = JSON.parse(data);
                console.log(details);
                if(details=="error"){
                  $('#meetings').append('<div class="col  s4"><div class="card-panel blue-grey"><span class=white-text><p><strong><b>No Requests</b></strong><p id=school></span></div></div></div>');
                }else
                {
      ...
      var from=details[i][0];
        ...
        var str='<div class="col s4"><div class="card-panel blue-grey darken-1"><span class=white-text><p><strong><b>';
        str+='</b></strong></p><p>';
        str+=canddetails+'</p></span></div></div>';
        $('#meetings').append(str);
  }
